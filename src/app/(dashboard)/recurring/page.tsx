...
export default function RecurringPage() {
    const { recurringRules, categories, accounts, projects, addRecurringRule, updateRecurringRule, deleteRecurringRule, addTransaction, settings, isLoading } = useStore();
    ...
    const handleScan = async () => {
        ...
        try {
            const result = await recognizeRecurring(
                ...
                projects.filter(p => p.status === 'ACTIVE').map(p => p.name),
                scanText,
                settings.currency,
            ...
            if (result.success && result.data && result.data.length > 0) {
                ...
                // Match account
                if (data.accountName) {
                    const matchedAccount = accounts.find(a => a.name.toLowerCase() === data.accountName?.toLowerCase());
                    if (matchedAccount) setAccountId(matchedAccount.id);
                }

                // Match project
                if (data.projectName) {
                    const matchedProject = projects.find(p => p.name.toLowerCase() === data.projectName?.toLowerCase());
