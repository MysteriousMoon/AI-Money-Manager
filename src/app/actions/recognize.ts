...
9. **PAYMENT ACCOUNT**: Identify the payment method/account (e.g., "Visa 1234", "Cash", "Alipay"). Match it to one of the **Existing Accounts** if possible. If not found${defaultAccountName ? ', use the Default Payment Account.' : ', leave empty.'}
10. **PROJECT**: Analyze if the expense belongs to a specific project (e.g., "Renovation", "Business Trip"). Match it to one of the **Active Projects** if possible. If not clear${defaultProjectName ? ', use the Default Project.' : ', leave empty.'}
11. Return a strict JSON ARRAY of transaction objects.
...
- Text: "Paid $25 for lunch at Chipotle using Visa" → Return 1 object with category "Food", currency "USD", accountName="Visa Card"
- Text: "装修买油漆 500" (Active Projects: ["Renovation"]) → Return 1 object with category "Home Improvement", amount 500, projectName="Renovation"`;
