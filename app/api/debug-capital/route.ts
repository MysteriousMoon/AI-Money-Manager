...
export async function GET() {
    try {
        ...
        allIncome.forEach(t => {
            totalIncomeByCurrency[t.currencyCode] = (totalIncomeByCurrency[t.currencyCode] || 0) + t.amount;
        });

        const totalExpenseByCurrency: Record<string, number> = {};
        allExpense.forEach(t => {
            totalExpenseByCurrency[t.currencyCode] = (totalExpenseByCurrency[t.currencyCode] || 0) + t.amount;
        });
        ...
        const assetValues = investments.map(i => ({
            name: i.name,
            amount: i.currentAmount ?? i.initialAmount ?? 0,
        ...
        assetValues.forEach(asset => {
            const match = allExpense.find(t =>
                ...
                Math.abs(t.amount - asset.amount) < 1.0
            );
            if (match) {
